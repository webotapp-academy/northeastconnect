...
export default function CommentSection({
  ...
  return (
    <div className="bg-white rounded-3xl p-6 md:p-8 shadow-sm border border-gray-100 mt-10">
      {/* Toast Alert */}
      {successToast && (
        <div className="fixed bottom-6 right-6 z-50 bg-emerald-900 text-white px-5 py-3 rounded-2xl shadow-2xl flex items-center gap-3 animate-in slide-in-from-bottom-5 duration-300">
          ...
          <div className="flex-1">
            <div className="relative">
              <textarea
                ...
                className="w-full px-4 py-3 bg-gray-50 hover:bg-gray-50/80 focus:bg-white rounded-2xl border border-gray-200 focus:border-emerald-500 focus:ring-2 focus:ring-emerald-200 outline-none text-sm text-gray-900 transition-all placeholder:text-gray-400 resize-none"
              />
            </div>
            ...
            <div className="flex items-center justify-between mt-2">
              ...
              <button
                ...
                className="px-5 py-2.5 bg-gradient-to-r from-emerald-600 to-teal-700 hover:from-emerald-700 hover:to-teal-800 text-white text-sm font-semibold rounded-xl shadow-md hover:shadow-lg transition-all disabled:opacity-40 disabled:cursor-not-allowed cursor-pointer flex items-center gap-1.5"
              >
      ...
      {/* Comments List */}
      {loading ? (
        <div className="py-8 text-center text-gray-400 text-sm">Loading community comments...</div>
      ) : comments.length === 0 ? (
        <div className="text-center py-12 px-4 bg-gray-50 rounded-2xl border border-dashed border-gray-200">
          <div className="text-3xl mb-2">🏔️</div>
          <h4 className="font-semibold text-gray-700 text-base">No comments yet</h4>
            ...
            <div key={comment.id} className="group">
              <div className="flex items-start gap-3.5">
                ...
                <div className="flex-1 min-w-0">
                  <div className="bg-gray-50/80 hover:bg-gray-50 rounded-2xl p-4 border border-gray-100 transition-colors">
                    <div className="flex items-center justify-between flex-wrap gap-2 mb-1.5">
                      <div className="flex items-center gap-2 flex-wrap">
                        <Link
                          href={`/profile/${comment.user.username}`}
                          className="font-bold text-sm text-gray-900 hover:text-emerald-700 transition-colors"
                        >
                          {comment.user.fullName || `@${comment.user.username}`}
                        </Link>
                        <span className="text-xs text-gray-400 font-mono">@{comment.user.username}</span>
                        <RankBadge
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        {currentUser && (currentUser.id === comment.user.id || currentUser.role === "Admin") && (
                          <button
                            ...
                            className="text-gray-400 hover:text-red-600 text-xs transition-colors p-1 cursor-pointer"
                  ...
                  <div className="flex items-center gap-4 mt-2 px-2 text-xs">
                    ...
                    >
                      <svg
                        className={`w-4 h-4 ${comment.isLiked ? "fill-rose-600 text-rose-600" : ""}`}
                        fill="none"
                        viewBox="0 0 24 24"
                        stroke="currentColor"
                      >
                    ...
                    <button
                      ...
                      className="text-gray-500 hover:text-emerald-700 font-medium inline-flex items-center gap-1 cursor-pointer transition-colors"
                    >
                      <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    ...
                    <div className="mt-3 pl-4 border-l-2 border-emerald-400">
                      <div className="flex gap-2">
                        <input
                          ...
                          className="flex-1 px-3.5 py-2 bg-white rounded-xl border border-gray-300 focus:border-emerald-500 focus:ring-2 focus:ring-emerald-200 outline-none text-xs text-gray-900"
                        />
                        <button
                          onClick={() => handlePostReply(comment.id)}
                          disabled={submittingReply || !replyText.trim()}
                          className="px-3.5 py-2 bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-semibold rounded-xl disabled:opacity-50 cursor-pointer"
                        >
                          {submittingReply ? "..." : "Send"}
                        </button>
                        <button
                          onClick={() => setActiveReplyId(null)}
                          className="px-2 py-2 text-xs text-gray-500 hover:text-gray-700 cursor-pointer"
                        >
                          Cancel
                        </button>
                      </div>
                  ...
                  {comment.replies && comment.replies.length > 0 && (
                    <div className="mt-4 pl-4 sm:pl-6 border-l-2 border-emerald-100 space-y-3.5">
                              ...
                              <div className="flex items-center justify-between flex-wrap gap-1 mb-1">
                                <div className="flex items-center gap-1.5 flex-wrap">
                                  <Link
                                    ...
                                    className="font-bold text-xs text-gray-900 hover:text-emerald-700"
                                  >
                                    {reply.user.fullName || `@${reply.user.username}`}
                                  </Link>
                                  <span className="text-[11px] text-gray-400 font-mono">@{reply.user.username}</span>
                                  <RankBadge
                            ...
                            <div className="flex items-center gap-3 mt-1 px-2 text-[11px]">
                              <button
                                ...
                                className={`inline-flex items-center gap-1 transition-colors cursor-pointer ${
                                  reply.isLiked ? "text-rose-600 font-semibold" : "text-gray-400 hover:text-rose-600"
                                }`}
                              >
                                <svg
                                  className={`w-3.5 h-3.5 ${reply.isLiked ? "fill-rose-600 text-rose-600" : ""}`}
                                  fill="none"
                                  viewBox="0 0 24 24"
                                  stroke="currentColor"
                                >
