...
interface PageProps {
  searchParams: Promise<{ district?: string; type?: string; term?: string; state?: string }>;
}

export default async function AdventurePage({ searchParams }: PageProps) {
  const { district = "", type = "", term = "", state = "" } = await searchParams;
      ...
      <div className="container mx-auto px-2 sm:px-4 max-w-7xl">
        ...
        <div className="mb-4 bg-white/75 dark:bg-slate-900/75 backdrop-blur-xl border border-white/60 dark:border-slate-800/80 rounded-2xl p-2.5 sm:p-3 shadow-sm">
          <div className="flex items-center gap-2 overflow-x-auto pb-1 scrollbar-none snap-x">
            {NE_STATES.map((st) => {
              ...
              const href = st.name === "All States" ? "/adventure" : `/adventure?state=${encodeURIComponent(st.name)}`;
              return (
                <Link
                  key={st.name}
                  href={href}
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div>
              ...
              <h1 className="text-xl sm:text-2xl font-black text-slate-900 dark:text-slate-100 tracking-tight">
                Outdoor Adventures &amp; Expeditions
              </h1>
              <p className="text-xs sm:text-sm text-slate-500 dark:text-slate-400 mt-0.5">
                River rafting, mountain trekking, caving, paragliding, and camping spots across Northeast India
              </p>
            </div>
            ...
            <div className="text-xs font-medium shrink-0">
              <span className="px-3.5 py-1.5 bg-slate-100/90 dark:bg-slate-800/90 rounded-full border border-slate-200 dark:border-slate-700/80 text-slate-700 dark:text-slate-300 shadow-xs">
                <strong className="text-slate-900 dark:text-slate-100 font-bold">{adventures.length}</strong> Expeditions
              </span>
            </div>
          </div>

          {/* Search & Filter Bar */}
          <form action="/adventure" method="GET" className="mt-5 pt-4 border-t border-slate-100 dark:border-slate-800/80 flex flex-col sm:flex-row gap-2.5">
            ...
            <div className="flex-1 relative">
              ...
              <input
                ...
                name="term"
                defaultValue={term}
                placeholder="Search river rafting, trekking trails, caving..."
        ...
        {adventures.length === 0 ? (
          <div className="bg-white/75 dark:bg-slate-900/75 backdrop-blur-xl border border-dashed border-slate-300 dark:border-slate-800 rounded-3xl p-12 text-center shadow-sm">
            ...
            <h3 className="text-base font-bold text-slate-900 dark:text-slate-100 mb-1">No adventure activities found</h3>
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
            {adventures.map((item) => {
              ...
              let mainImage = "https://images.unsplash.com/photo-1533240332313-0db49b459ad6?w=900&auto=format&fit=crop&q=60";
              if (item.imageUrls) {
                try {
                  ...
                  <div>
                    ...
                    <div className="p-5 space-y-2">
                      ...
                      <div className="flex items-center gap-1 text-xs text-slate-500 dark:text-slate-400 pt-1 font-medium">
                        ...
                        <span className="truncate">{item.district || item.location || "Northeast India"}</span>
                      </div>
                    </div>
                  </div>
